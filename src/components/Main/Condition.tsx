...
import {
  ...
  updateCondition,
  removeCondition,
} from "@store/slices/conditions.slice";
...
const Condition = ({
  ...
  condition,
  isLoading,
}: ConditionComponentProps & typeof defaultConditionProps) => {
  ...
  const [valueHelper, setValueHelper] = useState<string>(
    ConditionValueHelpers.valid
  );
  const [isMouseHoverAddButton, setIsMouseHoverAddButton] =
  ...
  useEffect(() => {
    ...
    if (operator === "greater" || operator === "less") {
      if (isValidNumber(String(value))) updatedVal = Number(value);
        ...
        <FormControl fullWidth>
          {isLoading ? (
            ...
            <Fragment>
              ...
              >
                {Object.keys(Operators).map((field: string) => (
                  <MenuItem key={field} value={field}>
                    {(Operators as any)[field]}
                  </MenuItem>
                ))}
              </Select>
            </Fragment>
          ...
          {isLoading ? (
            ...
            <Fragment>
              <TextField
                ...
                label="Value"
                variant="outlined"
                error={valueHelper !== ConditionValueHelpers.valid}
                placeholder="Value"
                value={value}
                onChange={handleChangeValue}
              />
              {valueHelper !== ConditionValueHelpers.valid && (
