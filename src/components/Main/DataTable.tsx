...
const DataTable = ({
  isLoading,
}: DataTableComponentProps & typeof defaultDataTableProps) => {
    ...
    filteredTableData = tableData.filter((row) => {
      ...
      for (orConditions of conditions) {
        ...
        for (orCondition of orConditions) {
          if (orCondition.value === "") continue;

          switch (orCondition.operator) {
            case "equal":
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              isOrConditionsValid ||=
                row[orCondition.field as string] === orCondition.value;

              break;
            case "greater":
              if (!isValidNumber(orCondition.value)) continue;

              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              isOrConditionsValid ||=
                Number(row[orCondition.field as string]) > orCondition.value;
            ...
            case "contain":
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              isOrConditionsValid ||= row[orCondition.field as string].includes(
                orCondition.value
              );
            ...
            case "notcontain":
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              isOrConditionsValid ||= !row[
                orCondition.field as string
              ].includes(orCondition.value);

              break;
            case "regex":
              if (!isValidRegex(orCondition.value as string)) continue;

              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              isOrConditionsValid ||= new RegExp(
                orCondition.value as string
              ).test(row[orCondition.field as string]);
  ...
  return (
    <Fragment>
      <Box sx={{ mt: 6, mb: 3 }}>
        ...
        <Chip
          label={`Filtered: ${isLoading ? "-" : filteredTableData.length}`}
          color="primary"
          sx={{ ml: 2 }}
